...
- Docker with buildx.
- A SporeVM checkout at `../sporevm`, or `SPOREVM_REPO=/path/to/sporevm`.
- Alternatively, a prebuilt `spore` binary via `SPORE_BIN=/path/to/spore` or on
  `PATH`.

When a SporeVM checkout is available, the scripts build it before running unless
`--no-build` is passed.
...
../sporevm/zig-out/bin/spore rootfs import-oci /tmp/sporevm-rails-rspec.oci \
...
```bash
bash rails/bin/fanout-rspec --count 8 --memory-mib 4096
...
bash rails/bin/fanout-rspec --spore-repo /path/to/sporevm
bash rails/bin/fanout-rspec --spore-bin /path/to/spore --no-build
